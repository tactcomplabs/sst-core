...
private:
    ...
    NumericHandle(ObjTreeCont* n, Kind k) : node_(n), kind_(k) {}
...
public:
    ...
    bool operator<(T rhs) const { return cmpConst(rhs, std::less<>{}); }

    template<typename T, typename = std::enable_if_t<std::is_arithmetic_v<T>>>
    bool operator>(T rhs) const { return cmpConst(rhs, std::greater<>{}); }

    template<typename T, typename = std::enable_if_t<std::is_arithmetic_v<T>>>
    bool operator<=(T rhs) const { return cmpConst(rhs, std::less_equal<>{}); }

    template<typename T, typename = std::enable_if_t<std::is_arithmetic_v<T>>>
    bool operator>=(T rhs) const { return cmpConst(rhs, std::greater_equal<>{}); }

    template<typename T, typename = std::enable_if_t<std::is_arithmetic_v<T>>>
    bool operator==(T rhs) const { return cmpConst(rhs, std::equal_to<>{}); }

    template<typename T, typename = std::enable_if_t<std::is_arithmetic_v<T>>>
    bool operator!=(T rhs) const { return cmpConst(rhs, std::not_equal_to<>{}); }
    ...
    bool cmpHandle(const NumericHandle& rhs, Cmp cmp) const {
        ...
        auto visitRhs = [&](auto lhsVal) -> bool {
            auto inner = [&](auto rhsVal) -> bool {
                if constexpr (std::is_integral_v<decltype(lhsVal)> &&
                              std::is_integral_v<decltype(rhsVal)>) {
                    ...
                    return cmp(static_cast<long double>(lhsVal),
                               static_cast<long double>(rhsVal));
                }
            };
...
template<typename T, typename = std::enable_if_t<std::is_arithmetic_v<T>>>
bool operator<(T lhs, const NumericHandle& rhs)  { return rhs > lhs; }
